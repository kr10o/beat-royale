...
// .wrangler/tmp/bundle-tcVAM1/strip-cf-connecting-ip-header.js
...
var BattleLobbyDO = class extends DurableObject {
  env;
  battleId;
  phase = "idle";
  polls = { 1: 0, 2: 0 };
...
__name(BattleLobbyDO, "BattleLobbyDO");
...
app.post("/api/auth/login", async (c) => {
  ...
  try {
    ...
    return c.json({ success: true, userId: user.id, displayName: profile3?.display_name || "Producer" });
  } catch (err) {
...
app.get("/api/auth/me", async (c) => {
  ...
  try {
    ...
    return c.json({ authenticated: true, user: { id: payload.id, role: payload.role, profile: profile3 } });
  } catch (e) {
    return c.json({ authenticated: false }, 401);
  }
});
app.get("/api/profiles/:id", async (c) => {
  ...
  try {
    ...
    return profile3 ? c.json(profile3) : c.notFound();
  } catch (err) {
...
// .wrangler/tmp/bundle-tcVAM1/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-tcVAM1/middleware-loader.entry.ts
